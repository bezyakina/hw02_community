...
class TestPost:
    def test_post_model(self):
        ...
        ), "Добавьте дату и время проведения события `pub_date` модели `Post`"
        assert (
...
class TestGroupView:
    ...
    def test_group_view(self, client, post_with_group):
        try:
            ...
            assert (
                False
            ), f"""Страница `/group/<slug>/` работает неправильно. Ошибка: `{e}`"""
        ...
        assert re.search(
            ...
            + post_with_group.pub_date.strftime("%d %b %Y"),
            html,
